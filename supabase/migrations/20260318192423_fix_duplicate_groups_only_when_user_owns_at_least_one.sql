/*
  # Fix get_user_duplicate_invoice_groups - only show duplicates when user owns at least one

  The previous fix still showed users duplicates from their department even when
  they didn't personally own or approve any invoice in the group.

  New rule: a duplicate group is shown to the user ONLY IF:
  - The user is uploaded_by OR current_approver_id of at least one invoice in that group
  - AND there exist at least 2 invoices total with that same key (across all they can see)

  Department membership still provides visibility into the other invoice in the group,
  but the group only appears if the user has a personal stake in it.
*/
...
BEGIN
  ...
  ELSE
    -- For non-admin users:
    -- 1. Find group keys where the user is personally involved (uploaded_by or current_approver)
    -- 2. For those keys, show ALL invoices in the group that the user can access
    --    (own invoices + department member invoices)
    -- This means the user only sees duplicates for invoices they personally own/approve,
    -- but can see all duplicates within that group (including ones from their dept)
    RETURN QUERY
    WITH
    -- All invoices with their group key
    all_keyed AS (
      SELECT
        i.id AS inv_id,
    ...
    -- Group keys where this user is personally involved (uploaded_by or current_approver)
    user_owned_keys AS (
    ...
    -- Among those keys, find ones that have at least 2 invoices total (duplicates)
    dup_keys AS (
      SELECT ak.grp_key
      FROM all_keyed ak
    ...
    -- All invoice IDs in those duplicate groups that the user can access
