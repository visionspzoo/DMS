/*
  # Fix audit log NULL description issue

  ## Problem
  - log_invoice_update trigger is creating audit logs with NULL description
  - This happens when field values are NULL and string concatenation returns NULL
  - PostgreSQL string concatenation with NULL returns NULL

  ## Solution
  - Use COALESCE to handle NULL values in string concatenation
  - Ensure description is never NULL when audit log is created
*/
...
BEGIN
  ...
  IF OLD.status IS DISTINCT FROM NEW.status THEN
    ...
    v_description := 'Status zmieniony z "' || COALESCE(OLD.status, 'brak') || '" na "' || COALESCE(NEW.status, 'brak') || '"';
  ...
  IF OLD.gross_amount IS DISTINCT FROM NEW.gross_amount THEN
    IF v_action IS NULL THEN
      ...
      v_description := 'Kwota brutto zmieniona z ' || COALESCE(OLD.gross_amount::text, 'brak') || ' na ' || COALESCE(NEW.gross_amount::text, 'brak');
    ELSE
      v_description := v_description || '; Kwota brutto zmieniona z ' || COALESCE(OLD.gross_amount::text, 'brak') || ' na ' || COALESCE(NEW.gross_amount::text, 'brak');
    END IF;
  ...
  IF OLD.description IS DISTINCT FROM NEW.description THEN
    ...
    ELSE
      v_description := COALESCE(v_description, '') || '; Opis zmieniony';
    END IF;
  ...
  IF v_action IS NOT NULL AND v_description IS NOT NULL THEN
