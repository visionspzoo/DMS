/*
  # Fix ambiguous column reference "id" in get_user_duplicate_invoice_groups

  The EXISTS subquery in the non-admin branch had an ambiguous "id" column reference.
  Fix by using fully qualified table aliases throughout.
*/
...
BEGIN
  ...
  ELSE
    ...
    dup_ids AS (
      ...
      INNER JOIN user_accessible_keys uak ON aig.grp_key = uak.grp_key
      WHERE (
        SELECT count(*) FROM all_in_groups aig2 WHERE aig2.grp_key = aig.grp_key
      ) >= 2
    )
    SELECT
