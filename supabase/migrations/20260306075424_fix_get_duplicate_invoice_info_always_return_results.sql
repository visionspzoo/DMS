/*
  # Fix get_duplicate_invoice_info - Always Return Duplicate Data

  ## Problem
  The function was returning {"error": "not_authenticated"} when auth.uid() was null,
  causing the frontend to silently show no duplicates. It also had a broken v_can_access
  check that always returned true (SECURITY DEFINER bypasses RLS), making the
  accessible/inaccessible distinction meaningless.

  ## Fix
  - Remove the auth.uid() check - since SECURITY DEFINER already bypasses RLS,
    anyone who can call this RPC (authenticated users) should see all duplicate info
  - Remove the broken v_can_access sub-query (it always returned true anyway)
  - Always mark duplicates as accessible=true and return full info
  - This satisfies the requirement: "everyone should see duplicate info"

  ## Security
  - Function is still SECURITY DEFINER (required to bypass RLS for cross-department lookup)
  - GRANT EXECUTE TO authenticated ensures only logged-in users can call it
*/
