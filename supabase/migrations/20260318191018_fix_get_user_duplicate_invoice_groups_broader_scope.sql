/*
  # Fix get_user_duplicate_invoice_groups to show duplicates for all accessible invoices

  ## Problem
  The function only searched for duplicates among invoices where uploaded_by = caller.
  Users who are current_approver_id or department members could not see duplicates
  of invoices they can access.

  ## Changes
  - For regular users: find all invoice groups where the user has access to at least
    one invoice (via uploaded_by, current_approver_id, or department membership)
...
BEGIN
  ...
  ELSE
    -- Regular users: find groups where they have access to at least one invoice
    -- Access = uploaded_by, current_approver_id, or department membership
    RETURN QUERY
    WITH user_accessible_invoices AS (
      SELECT i.id,
      ...
      WHERE i.invoice_number IS NOT NULL AND i.invoice_number <> ''
        AND (
          ...
          OR EXISTS (
            ...
            WHERE dm.department_id = i.department_id
    ...
    user_grp_keys AS (
      SELECT DISTINCT grp_key FROM user_accessible_invoices
    ),
    dup_ids AS (
