/*
  # Fix get_user_duplicate_invoice_groups - accept optional user_id parameter
  
  auth.uid() may return NULL in certain SECURITY DEFINER contexts with row_security=off.
  We add a p_user_id parameter so the frontend can pass the user ID directly,
  while falling back to auth.uid() if not provided.
  Also recreate without SET row_security TO off in proconfig (use SET LOCAL instead).
*/

DROP FUNCTION IF EXISTS public.get_user_duplicate_invoice_groups();
