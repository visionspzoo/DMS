/*
  # Debug: dodaj logowanie do handle_invoice_approval

  Tymczasowe logowanie żeby zrozumieć dlaczego trigger nie działa gdy
  wywołany przez frontend (z auth.uid() ustawionym na Kierownika).
*/
...
BEGIN
    ...
    IF NEW.status = 'waiting' AND OLD.status = 'draft' THEN
        ...
        INSERT INTO audit_logs (invoice_id, user_id, action, new_values, description)
        VALUES (
            NEW.id, auth.uid(), 'debug_trigger_draft_waiting',
            jsonb_build_object('v_approver_role', v_approver_role, 'v_approver_dept_id', v_approver_dept_id),
            'DEBUG: in draft->waiting block'
        );
        ...
        IF v_approver_role = 'Kierownik' THEN
            ...
            INSERT INTO audit_logs (invoice_id, user_id, action, new_values, description)
            VALUES (
                NEW.id, auth.uid(), 'debug_limits_check',
                jsonb_build_object('v_invoice_amount', v_invoice_amount, 'limits_check', v_limits_check),
                'DEBUG: limits check result'
            );
